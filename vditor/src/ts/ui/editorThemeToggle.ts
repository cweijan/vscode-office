...
let themeToggleBtn: HTMLButtonElement | undefined;
let boundVditor: IVditor | undefined;

export const initEditorThemeToggle = (vditor: IVditor) => {
    boundVditor = vditor;
    if (themeToggleBtn) {
        updateEditorThemeToggle(vditor.options.editorTheme || "Auto");
        return;
    }
    ...
    themeToggleBtn.addEventListener("click", () => {
        if (!boundVditor || !themeToggleBtn) {
            return;
        }
        const theme = themeToggleBtn.dataset.theme || "Auto";
        const nextTheme = theme === "Light" ? "Auto" : "Light";
        setEditorTheme(boundVditor, nextTheme);
    });
...
export const updateEditorThemeToggle = (theme: string) => {
    ...
    const title = isLight
        ? (window.VditorI18n?.["editor-theme-toggle-to-auto"] || "Switch to Auto (follow VS Code)")
        : (window.VditorI18n?.["editor-theme-toggle-to-light"] || "Switch to Light theme");
